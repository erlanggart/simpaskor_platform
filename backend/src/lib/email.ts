...
import { sendAndLog } from "../utils/mailLog";
...
export async function sendTicketEmail(params: {
	...
	attendees?: { name: string; email: string; phone?: string | null; ticketCode: string; supportTeam?: string | null; qrBase64?: string }[];
}) {
          ...
          <tr><td style="padding:16px;">
            <div style="background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;padding:16px;margin-bottom:${idx < (params.attendees?.length || 0) - 1 ? '8' : '0'}px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                ...
                ${att.supportTeam ? `<tr><td style="padding:4px 0;color:#6b7280;font-size:13px;">Menonton Pasukan</td><td style="padding:4px 0;color:#111827;font-size:13px;font-weight:600;text-align:right;">${att.supportTeam}</td></tr>` : ''}
              </table>
...
export async function sendTicketEmailFromServer(params: {
	...
	totalAmount: number;
	ticketDescription?: string | null;
	attendees?: { name: string; email: string; phone?: string | null; ticketCode: string; supportTeam?: string | null }[];
}) {
