...
// Delete user — disabled. Accounts are never hard-deleted; use the ACTIVE/INACTIVE
// status to enable or disable an account instead.
router.delete(
	"/:userId",
	authenticate,
	requireSuperAdmin,
	async (_req: AuthenticatedRequest, res: Response) => {
		return res.status(403).json({
			error: "User deletion disabled",
			message:
				"Akun tidak dapat dihapus. Nonaktifkan akun dengan mengubah status menjadi INACTIVE.",
		});
	}
);
