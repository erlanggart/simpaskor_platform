...
const ADMIN_FEE_SOURCES = ["ticket", "voting", "registration"] as const;

const parseAdminFeeDateQuery = (value: unknown, fieldName: string, endOfDay = false): Date | null => {
...
// GET /api/admin/admin-fees - Internal admin fee ledger for SuperAdmin
router.get(
	"/admin-fees",
	authenticate,
	authorize("SUPERADMIN"),
	async (req: AuthenticatedRequest, res: Response) => {
		try {
			const from = parseAdminFeeDateQuery(req.query.from, "from");
			const to = parseAdminFeeDateQuery(req.query.to, "to", true);
			...
			if (source && !ADMIN_FEE_SOURCES.includes(source as typeof ADMIN_FEE_SOURCES[number])) {
				return res.status(400).json({ error: "source harus ticket, voting, atau registration" });
			}
			...
			const [total, totalsBySource, rows] = await Promise.all([
				prisma.adminFeeTransaction.count({ where }),
				prisma.adminFeeTransaction.groupBy({
					by: ["source"],
					where,
					_sum: { adminFee: true, baseAmount: true },
					_count: { id: true },
				}),
				prisma.adminFeeTransaction.findMany({
					where,
					orderBy: { paidAt: "desc" },
					skip,
					take: limit,
			...
			const sourceSummary = {
				ticket: { adminFee: 0, baseAmount: 0, count: 0 },
				voting: { adminFee: 0, baseAmount: 0, count: 0 },
				registration: { adminFee: 0, baseAmount: 0, count: 0 },
			};

			for (const item of totalsBySource) {
				if (item.source in sourceSummary) {
					sourceSummary[item.source as keyof typeof sourceSummary] = {
						adminFee: toNumber(item._sum.adminFee),
						baseAmount: toNumber(item._sum.baseAmount),
			...
			const totalAdminFee =
				sourceSummary.ticket.adminFee +
				sourceSummary.voting.adminFee +
				sourceSummary.registration.adminFee;
			const totalBaseAmount =
				sourceSummary.ticket.baseAmount +
				sourceSummary.voting.baseAmount +
				sourceSummary.registration.baseAmount;
			...
			res.json({
				...
				summary: {
					totalAdminFee,
					totalBaseAmount,
					ticketAdminFee: sourceSummary.ticket.adminFee,
					votingAdminFee: sourceSummary.voting.adminFee,
					registrationAdminFee: sourceSummary.registration.adminFee,
				},
				counts: {
					total,
					tickets: sourceSummary.ticket.count,
					voting: sourceSummary.voting.count,
					registrations: sourceSummary.registration.count,
				},
				pagination: {
					page,
					limit,
			...
			console.error("Error fetching admin fee ledger:", error);
			res.status(500).json({ error: "Gagal mengambil data admin fee" });
		}
	}
);
