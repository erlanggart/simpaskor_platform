		...
		return (
			<div className="h-screen flex flex-col overflow-hidden">
				...
				<div className="flex-1 flex flex-col lg:flex-row w-full min-h-0">
					{/* LEFT: Image upload (2/3) — top half image, bottom half form */}
					...
					{/* RIGHT: Slide list with reorder (1/3) */}
					<div className="lg:w-1/3 flex flex-col lg:border-l border-gray-200/50 dark:border-white/[0.06] min-h-0 bg-white/40 dark:bg-white/[0.01]">
						...
						<div className="flex-1 overflow-y-auto p-2">
							{editingGuideSlides.slides.map((s, sIdx) => (
								<React.Fragment key={s.id}>
									{/* Insert button before each slide */}
									<button
									...
									>
										<LuGripVertical className={`w-3 h-3 flex-shrink-0 ${sIdx === activeSlideIdx ? "text-white/50" : "text-gray-300 dark:text-gray-600"}`} />

										{/* Thumbnail */}
										<div className="w-10 h-7 rounded bg-gray-100 dark:bg-white/[0.04] overflow-hidden flex-shrink-0">
											{s.imageUrl ? (
												<img src={s.imageUrl} alt="" className="w-full h-full object-cover" />
											) : (
												<div className="w-full h-full flex items-center justify-center">
													<LuImage className={`w-3 h-3 ${sIdx === activeSlideIdx ? "text-white/40" : "text-gray-300 dark:text-gray-600"}`} />
												</div>
											)}
										</div>
										...
										{/* Title */}
										<div className="flex-1 min-w-0">
											<p className={`text-[11px] font-semibold truncate ${sIdx === activeSlideIdx ? "text-white" : ""}`}>
												{sIdx + 1}. {s.title}
											</p>
										</div>
